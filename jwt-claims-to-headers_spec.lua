...
for _, strategy in helpers.each_strategy() do
  describe(
    "Jwt-Claims-to-Headers-Plugin: (access) [#" .. strategy .. "]",
    function()
      local client
      ...
      setup(
        function()
          ...
          create_route(
            "test-claims-to-headers-table.com",
            {},
            {
          ...
          print(
            "Created jwt_secret id " ..
              jwt_secret.id ..
                ", at epoch time " ..
                  jwt_secret.created_at .. ", key " .. jwt_secret.key .. ", secret " .. jwt_secret.secret
          )
      ...
      describe(
        ...
        function()
          it(
            "with the 'jwt' query parameter, contains the X-claims header",
            function()
              local r =
                assert(
                client:send {
          ...
          it(
            "with a jwt as the Bearer token, contains the X-claims header",
            function()
              local r =
                assert(
                client:send {
      ...
      describe(
        "request with config parameters",
        function()
          it(
            "with the 'prefix' config parameter, uses the prefix in the header names",
            function()
              local r =
                assert(
                client:send {
          ...
          it(
            "with the 'claims_to_headers_table' config parameter, uses the custom headers",
            function()
              local r =
                assert(
                client:send {
          ...
          it(
            "with uri_param_names specifying the location of the jwt, contains the X-claims header",
            function()
              local r =
                assert(
                client:send {
